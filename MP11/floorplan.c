#include "floorplan.h"
//Through the use of recursion, some structs, and a bunch of
//pointers, this program implements a floorplanner that organizes
//a number of rectangular modules in such a way that the area
//of the combined modules is at a minumum. An important concept
//in this program is the idea of slicing, and this code can only
//be applied in situations involving a slicing floorplan. This is
//made possible due to the coder's understanding of a binary tree.
...
{
//Leaf nodes have no children, so left/right pointers are null
...
{
//An internal node means that it is an H or V. If it is not an H or V,
//it is reffered to as an UNDEFINED_CUTLINE.
...
{
node_t* temp;
//a is root, b is below.
temp = b;
//Start at b, and traverse it's parent, it's parent's parent, etc until you hit
//either 1) a or 2) the root. If you get all the way to the root and it is still not
//equal to a, then it is NOT in the subtree. 
  while(temp->parent != a && temp->parent != NULL)
  {
    temp = temp->parent;
  }
//statement for NOT FOUND ---- hits the root.
  if(temp->parent == NULL && temp != a)
    return 0;
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
node_t* tempA;
//Only need to change the module pointers, using a temp node_t for swapping
  tempA->module = a->module;
  a->module = b->module;
  b->module = tempA->module;
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
node_t* temp;
//THREE POINTERS MUST BE UPDATED
...
//Process the data part of root element (or current element)
  if(ptr->cutline == V)
  {
    (expression+*nth)->cutline = V;
    (expression+*nth)->module = NULL;
  }
  else if(ptr->cutline == H)
  {
    (expression+*nth)->cutline = H;
    (expression+*nth)->module = NULL;
  }
  else // data is NOT a cutline
  {
    (expression+*nth)->module = ptr->module;
    (expression+*nth)->cutline = UNDEFINED_CUTLINE;
  }

//Increment the position where the postfix expression needs to fill
  *nth = *nth + 1;
}
...
node_t* init_slicing_tree(node_t* par, int n)
{
  ...
  // TODO:
//This function ONLY creates two nodes at a time, NOT 3.
//It creates the internal node and the right child that
//are connected via the right diagonal line.

//Create the new node. This is the internal node except
//for when making the last call.
    node_t* ptr = (node_t*)malloc(sizeof(node_t));
//BASE CASE - reached last internal node on the left
  if(n == num_modules - 1)
  {
  //Only need to fill the left child
...
//INTERNAL NODE
  //Assign the data to the module pointed by ptr
    ptr->parent = par;
//Only want to assign as the left child when
...
//RIGHT CHILD
  ...
  //Creates the node to be used as the module
    node_t* rt = (node_t*)malloc(sizeof(node_t));
    rt->parent = ptr;
    ptr->right = rt;
    rt->right = NULL;
    rt->left = NULL;
    rt->cutline = UNDEFINED_CUTLINE;
    rt->module = &modules[n];
